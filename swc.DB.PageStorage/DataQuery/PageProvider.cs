...
namespace swc.DB.PageStorage.DataQuery
{
    public class PageProvider : IPageProvider
    {
        private readonly PageDbContext dbContext;
        private readonly ILogger<PageProvider> logger;
        private readonly IMapper autoMap;

        public PageProvider(PageDbContext dbContext, ILogger<PageProvider> logger, IMapper autoMap)
        {
        ...
        public async Task<(bool IsSuccess, IEnumerable<Model.Page> pages, string ErrorMsg)> GetPagesAsync()
        {
            try
            {
                ...
                if (pages?.Any() ?? false)
                {
                    logger?.LogInformation($"Returning {pages.Count} pages.");
                    var result = autoMap.Map<IEnumerable<Repository.Page>, IEnumerable<Model.Page>>(pages);

                    return (true, result, null);
                }
                return (false, null, $"No pages found");
            }
            catch (Exception e)
            {
        ...
        public async Task<(bool IsSuccess, Model.Page page, string ErrorMsg)> GetPageByIdAsync(Guid id)
        {
            try
            {
                // Get Pages by Domain
                var page = await dbContext.Page.FirstOrDefaultAsync(p => p.Id == id)
                    // Include Links
                    // Include Content
                    ;
                if (page != null)
                {
                    ...
                    var result = autoMap.Map<Repository.Page, Model.Page>(page);
                    return (true, result, null);
                }
                return (false, null, $"No page found at -{id}-");
            }
            catch (Exception e)
            {
                logger?.LogError(e.ToString());
        ...
        public async Task<(bool IsSuccess, IEnumerable<Model.Page> pages, string ErrorMsg)> GetPagesInDomainAsync(string domain)
        {
            try
            {
                // Get Pages by Domain
                ...
                return (false, null, $"No pages found for domain -{domain}-");
            }
            catch (Exception e)
            {
                logger?.LogError(e.ToString());
        ...
        private void DemoPageContent()
        {
            if (!dbContext.Page.Any())
            {
                // If on initilization we don't have any data create some dummy data to test with, serves as a cheap mock
                dbContext.Page.Add(new Page()
                {
                    Id = Guid.NewGuid(),
                    Domain = "fivesecondtest.com",
                    ResourceUrl = "https://fivesecondtest.com/",
                    ResourceLocation = "/",
                    RawContent =
